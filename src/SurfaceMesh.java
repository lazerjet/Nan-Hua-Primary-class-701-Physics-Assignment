...
/**
 * Class for rendering a 3D mesh (a 3d polyhedral surface) using Processing 
 * 
 * @author Luca Castelli Aleardi (INF421, 2016)
 *
 */
public class SurfaceMesh {
	...
	Show3DMesh view; // Processing 3d frame (where meshes are rendered)
	public Polyhedron_3<Point_3> polyhedron3D; // triangle mesh
	
	/**
	 * Create a surface mesh from an OFF file
	 */	
	public SurfaceMesh(Show3DMesh view, String filename) {
		this.view=view;

    	polyhedron3D=MeshLoader.getSurfaceMesh(filename); // input 3d mesh (half-edge representation)
